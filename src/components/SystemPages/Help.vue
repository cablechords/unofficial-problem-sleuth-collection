...
<div class="card">
  <div class="cardContent center">
    ...
    <section class="pageMenu">
      <a v-for="(label, key) in {newreader: 'New Readers', usage: 'How To Use<br>The Collection', faq: 'Troubleshooting,<br>FAQ'}" 
        :key="key"
        :href="`/help/${key}`" 
    ...
    <section class="cardBody helpSection" v-if="cardSelection == 'faq'">
      ...
      <section class="faq">
        <dl>
          ...
          <dd>This is usually caused by a faulty asset pack. Try running the built-in <a href="/validator">Asset Pack Validator</a> to check if the problem can be automatically identified. If you still have trouble try re-downloading and re-extracting the asset pack, then try again.</dd>

          <dt>Images load as expected, but flash pages just show a grey box with the words "Couldn't load plugin."</dt>
          <dd>This means the native Adobe Flash player is failing to run. The most common cause of this is downloading the wrong version of the app for your computer's architecture. If you're using Windows, try using the new universal version labeled "The-Unofficial-Homestuck-Collection-&lt;VERSION&gt;-win.exe" under <a href="https://github.com/GiovanH/unofficial-homestuck-collection/releases">releases</a> with no -x64 or -ia32 in the name.</dd>
      ...
      <section class="faq">
        <dl>
          ...
          <dd>Yes! Though only the desktop version supports native flash, we have had a fully-featured port of The Unofficial Homestuck Collection that runs fully in the browser, which means it works on any modern web browser. The web port has been available at <a href="https://homestuck.giovanh.com/">homestuck.giovanh.com</a> since the release at the <a href="https://www.youtube.com/watch?v=AlJvr-leT94">2023 SAHCon panel</a></dd>
